...
class PSPlot(FromHDF5):
    ...
    def __call__(self, frame, ax, *o, **kwo):
        ...
        if self.File is not None:
            try:
                p = -self.File.get_time(
                    os.path.basename(frame),
                    "x",
                    "y",
                    "z"
                )
                v = -self.File.get_time(
                    os.path.basename(frame),
                    "vx",
                    "vy",
                    "vz"
        ...
        if self.J is None:
            ax.text(
                ...
                (self._file.get_time(
                    os.path.basename(frame),
                    "time") if self.File is not None else map.time,
                 ),
...
def create_sub_PSMap(sub, parent):
    parser = sub.add_parser(
        'psplot',
        help="Plot the phase space of multiple files.",
        parents=parent
    )
    parser.set_defaults(
        func=PSPlot,
