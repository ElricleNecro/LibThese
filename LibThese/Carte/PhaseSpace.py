...
def LoadEdge(file, **kwargs):
    col1, col2 = np.loadtxt(file, unpack=True, **kwargs)

    res = np.zeros((col1.shape[0]+1,))
    res[:-1] = col1[:]
    res[1:] = col2[:]
...
class PhaseSpaceData(Filter):
    ...
    def __init__(    self,
        ...
        if nb_r_bin is None:
            ...
            self._r_bin = nb_r_bin
        if nb_v_bin is None:
            self._v_bin = self.nb_bin
        else:
            self._v_bin = nb_v_bin
        if nb_j_bin is None:
            self._j_bin = self.nb_bin
        else:
            self._j_bin = nb_j_bin
...
class PSPlot(PhaseSpaceData):
    def __init__(self, file, **kwargs):
        ...
        if self.AxsLog:
            if "r_max" in kwargs and kwargs["r_max"]:
                ...
                r_max = self.r.max()
            if "r_min" in kwargs and kwargs["r_min"]:
                r_min = kwargs["r_min"]
            else:
                r_min = self.r.min()

            if "v_max" in kwargs and kwargs["v_max"]:
                v_max = kwargs["v_max"]
            else:
                v_max = self.v.max()
            if "v_min" in kwargs and kwargs["v_min"]:
                v_min = kwargs["v_min"]
            else:
                v_min = self.v.min()
            ...
            self.nbbin = (
                    10.**np.linspace(
                            np.log10( r_min ),
                            np.log10( r_max ),
                            # np.log10(self._bins_r.min()),
                            # np.log10(self._bins_r.max()),
                            nb_point
                    ),
                    np.linspace(
                        v_min,
        ...
        self.hist, self.X, self.Y = np.histogram2d(
                        ...
                        # bins=(
                            # 10**np.linspace(
                                ...
                                # self.nbbin
                            # ),
                            # # np.linspace(self._bins_r.min(), self._bins_r.max(), self.nbbin),
                            # np.linspace(self._bins_v.min(), self._bins_v.max(), self.nbbin),
                            # # self._bins_r,
                            # # self._bins_v,
    ...
    def Plot(self, fig=None, ax=None, log=False, colorbar=False, vmin=None, vmax=None, cmap=None):
        ...
        cb = ax.pcolormesh(self.X, self.Y, to_plot, vmin=vmin, vmax=vmax, cmap=cmap)
