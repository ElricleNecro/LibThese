...
from   LibThese.old.rw                 import Read_File
...
def exec_list(motif, G=6.67e-11, Rmax=2.92668e+17, rsoft=0.0790899, nbbin=100, vexec="Verif", periodic=False, filetheo="King_dim.log", fileparam="ci_param.dat", exclude=None, retour=False, video=False, tmax=0.1, UT=9.77894e8, Td=8.02191e6, opang=0.0, Other=""):
	...
	if isinstance(motif, str):
		if os.path.isfile(motif):
			...
			tmp, motif = os.path.split(motif)
			if tmp is '':
				tmp = '.'
			liste = dir.SearchFile(motif, exclude=exclude, pdir=tmp)
			liste.sort()
	elif isinstance(motif, list):
		liste = motif
		liste.sort()
	else:
		raise TypeError("Type de motif (" + str(type(motif)) + ") non supporté...")
	print(liste)
...
def plot(Dens, Pot, Distrib, data=None, m=1.42478e+29, prefix="", aff=True, save=False, fileparam="ci_param.dat", filesave=("Densite.pdf", "Potentiel.pdf", "Anisotropie.pdf", "Temperature.pdf", "Distribution.pdf"), suppose=False, video=False, info="", nbbin=300, BoxSize=180.12*p2m, center=0.0, rnorm=1.0):
	...
	try:
		...
		import time
		print("Erreur avec le tracé de la densité.")
		log = time.strftime("%d-%m-%Y.%H%M%S")
		os.rename("Masse.dat", "Masse_bug" + log + ".dat")
		os.rename("Densite.dat", "Densite_bug" + log + ".dat")
		os.rename("Potentiel-tc.dat", "Potentiel-tc_bug" + log + ".dat")
		os.rename("Distribution.dat", "Distribution_bug" + log + ".dat")
		os.rename("Energie.dat", "Energie_bug" + log + ".dat")
	...
	fig6 = plt.figure(prefix + "Densité logarithmique")
	if not suppose:
		fig6.clf()
	ax6 = fig6.add_subplot(111)

	ax6.set_title("Densité Log" + info)
	ax6.set_xlabel(r'$\log(r)$')
	ax6.set_ylabel(r'$\rho(r)$')

	ax6.plot(Dens[:,4], Dens[:,5], '-')
	if data != None:
		ax6.loglog(data[:,0]/rnorm, data[:,3], 'r-')
	xaxe = ax6.get_xlim()
	yaxe = ax6.get_ylim()
	#ax6.set_xlim( (1e15,xaxe[1]) )
	ax6.set_ylim( (np.min(Dens[:,1]),yaxe[1]) )
	ax6.legend(('Densité de la simulation', 'Densité théorique'), 'lower left', shadow=True)
...
def PlotParam(name="TimeParam.dat", prefix="", Param=None, aff=True, save=False, info="", filelist=("Axial Ratio", "Viriel", "Temperature", "Anisotropie", "Rayon", "Vitesse du centre"), tmax=0.1, UT=9.77894e8, Td=8.02191e6):
	...
	axV.axis([0, tmax * (len(Param)+1) * UT / (len(x) * Td), -2.5, -0.1])
